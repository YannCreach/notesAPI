...
-- Activer RLS sur les tables concernées (au cas où)
...
-- Place: owner-based policies compatibles Auth0 (sub) et Supabase Auth (auth.uid())
...
-- place_has_tag: lecture libre, écritures réservées au propriétaire du place
...
CREATE POLICY place_tag_insert_owner ON "place_has_tag"
  FOR INSERT WITH CHECK (
    EXISTS (
      ...
      WHERE p.id = place_id
        AND p.user_id = coalesce(auth.jwt()->>'sub', auth.uid()::text)
    )
  );
...
CREATE POLICY note_tag_insert_owner ON "note_has_tag"
  FOR INSERT WITH CHECK (
    EXISTS (
      ...
      WHERE n.id = note_id
        AND n.user_id = coalesce(auth.jwt()->>'sub', auth.uid()::text)
    )
  );
